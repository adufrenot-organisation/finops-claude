...
function ceScoped(sid){
  ...
  return {orgs,groups,resources,consumption};
}
...
async function ceDeleteScenario(m){if(!confirm(`Supprimer le scénario « ${m.s.Nom} » et toute sa configuration ?`))return;const actions=[...m.consumption.map(c=>['RemoveRecord',T.claudeConsumption,c.id]),...m.resources.map(r=>['RemoveRecord',T.claudeResources,r.id]),...m.groups.map(g=>['RemoveRecord',T.claudeGroups,g.id]),...m.orgs.map(o=>['RemoveRecord',T.claudeOrgs,o.id]),['RemoveRecord',T.claudeScenarios,m.s.id]];await grist.docApi.applyUserActions(actions);CE_SELECTED_SCENARIO=0;await ceReload()}
