...
function ceRenderOverview(m,pctCap){
  ...
  const rows=m.groups.map(g=>{
    ...
    const reallocNet=incoming-outgoing;
    const remaining=currentCap-used;
    const variance=currentCap-allocated;
    const landing=Math.max(used,currentCap||allocated);
    const ratio=currentCap?used/currentCap:0;
    const state=used>currentCap&&currentCap>0?'danger':ratio>=.9?'danger':ratio>=.7?'warn':'ok';
    return {g,o,allocated,currentCap,used,incoming,outgoing,reallocNet,remaining,variance,landing,ratio,state};
  });
  ...
  const commitmentMonths=Math.max(1,+m.s.Duree_Engagement_Mois||12);
  const monthlyEstimate=allocatedTotal/commitmentMonths;
  const riskRows=rows.filter(x=>x.state!=='ok').length;
  body.innerHTML=`<div class="ce-layout"><article class="card"><div class="ce-hero"><div><h3>Pilotage budgétaire consolidé</h3><p>Budget initial, budget révisé après arbitrages et consommation réelle.</p></div></div><div class="kpis"><div class="kpi"><div class="v">${money(monthlyEstimate,'EUR')}</div><div class="l">Estimation de la conso (par mois)</div><div class="ce-muted" style="margin-top:6px">${money(allocatedTotal,'EUR')} ÷ ${commitmentMonths} mois</div></div><div class="kpi"><div class="v">${money(allocatedTotal,'EUR')}</div><div class="l">Affectation initiale</div></div><div class="kpi"><div class="v">${money(currentCaps,'EUR')}</div><div class="l">Budgets révisés</div></div><div class="kpi"><div class="v">${money(usedTotal,'EUR')}</div><div class="l">Consommation réelle</div></div><div class="kpi"><div class="v ${remainingGlobal<0?'negative':''}">${money(remainingGlobal,'EUR')}</div><div class="l">Reste à consommer avant blocage</div></div><div class="kpi"><div class="v ${riskRows?'negative':''}">${riskRows}</div><div class="l">Groupes à surveiller</div></div></div><div class="ce-bar"><i style="width:${Math.min(100,m.cap?usedTotal/m.cap*100:0)}%"></i></div><div class="ce-muted" style="margin:8px 0 16px">${m.cap?pct(usedTotal/m.cap):'—'} du plafond global consommé. Le plafond organisation est global sur la période : il n’est pas mensuel.</div><div class="tablewrap"><table><thead><tr><th>Groupe</th><th>Affectation initiale</th><th>Budget révisé</th><th>Consommé réel</th><th>Reste sur plafond</th><th>Écart vs budget initial</th><th>Réallocation nette</th><th>Atterrissage simulé</th><th>Risque</th></tr></thead><tbody>${rows.map(x=>`<tr><td><b>${esc(x.g.Nom)}</b><br><span class="ce-muted">${esc(x.o?.Nom||'—')}</span></td><td class="num">${money(x.allocated,'EUR')}</td><td class="num"><b>${money(x.currentCap,'EUR')}</b></td><td class="num">${money(x.used,'EUR')}<br><span class="ce-muted">${x.currentCap?pct(x.ratio):'—'} du plafond</span></td><td class="num ${x.remaining<0?'ce-danger':''}">${money(x.remaining,'EUR')}</td><td class="num ${x.variance>0?'ce-danger':''}">${x.variance>=0?'+':''}${money(x.variance,'EUR')}</td><td class="num">${x.reallocNet>=0?'+':''}${money(x.reallocNet,'EUR')}</td><td class="num">${money(x.landing,'EUR')}</td><td>${x.state==='danger'?'<span class="badge danger">Risque / dépassement</span>':x.state==='warn'?'<span class="badge warn">À surveiller</span>':'<span class="badge ok">Maîtrisé</span>'}</td></tr>`).join('')||'<tr><td colspan="9">Aucun groupe.</td></tr>'}<tr class="total"><td>TOTAL</td><td class="num">${money(allocatedTotal,'EUR')}</td><td class="num">${money(currentCaps,'EUR')}</td><td class="num">${money(usedTotal,'EUR')}</td><td class="num">${money(currentCaps-usedTotal,'EUR')}</td><td class="num">${money(currentCaps-allocatedTotal,'EUR')}</td><td class="num">${money(rows.reduce((a,x)=>a+x.reallocNet,0),'EUR')}</td><td class="num">${money(rows.reduce((a,x)=>a+x.landing,0),'EUR')}</td><td></td></tr></tbody></table></div><p class="ce-muted" style="margin-top:8px">Atterrissage simulé = niveau le plus élevé entre la consommation déjà constatée et le budget révisé. Il s’agit d’une vue de pilotage, pas d’une prévision statistique.</p></article><article class="card"><h3>Architecture budgétaire</h3><p>Répartition par organisation et groupe.</p>${m.orgs.map(o=>{const gs=m.groups.filter(g=>+g.Organisation===+o.id),used=gs.reduce((a,g)=>a+ceGroupUsed(m,g.id),0);return `<div class="ce-org"><b>${esc(o.Nom)}</b><span class="ce-muted"> · plafond global bloquant ${money(o.Plafond_Global,'EUR')}</span><div class="ce-muted">Consommé ${money(used,'EUR')} · reste avant blocage ${money((+o.Plafond_Global||0)-used,'EUR')}</div><div class="ce-groups">${gs.map(g=>{const rs=m.active.filter(r=>+r.Groupe===+g.id),ex=rs.reduce((a,r)=>a+ceLimit(r,m.groups),0);return `<div class="ce-group"><b>${esc(g.Nom)}</b><div class="ce-muted">${rs.length} ressource(s)</div><div>${money(g.Limite_User_Mois,'EUR')} / user / mois</div><div class="ce-muted">Exposition mensuelle utilisateurs : <b>${money(ex,'EUR')}</b></div></div>`}).join('')||'<span class="ce-muted">Aucun groupe</span>'}</div></div>`}).join('')||'<div class="ce-muted">Aucune organisation.</div>'}</article></div>`;
}
function ceRenderStructure(m){const body=document.getElementById('ceTabBody');body.innerHTML=`<div class="ce-layout"><article class="card"><div class="ce-hero"><div><h3>Organisations</h3><p>Plafonds globaux du scénario.</p></div>${ceCanEdit()?'<button id="ceAddOrg" class="btn primary">+ Organisation</button>':''}</div>${m.orgs.map(o=>{const gs=m.groups.filter(g=>+g.Organisation===+o.id),allocated=gs.reduce((a,g)=>a+Math.max(0,+g.Enveloppe_Allouee||0),0),used=gs.reduce((a,g)=>a+ceGroupUsed(m,g.id),0);return `<div class="ce-org"><div style="display:flex;justify-content:space-between;gap:8px"><div><b>${esc(o.Nom)}</b><div class="ce-muted">${money(o.Plafond_Global,'EUR')} / mois · ${esc(o.Mode_Facturation||'PO')} · enveloppe ${money(o.Enveloppe_Engagee,'EUR')}</div><div class="ce-muted">Réparti aux groupes : <b>${money(allocated,'EUR')}</b> · non affecté : <b>${money(Math.max(0,(+o.Enveloppe_Engagee||0)-allocated),'EUR')}</b> · consommé : <b>${money(used,'EUR')}</b></div></div>${ceCanEdit()?`<div><button class="mini-btn" data-ce-org="${o.id}">Modifier</button> <button class="mini-btn ce-danger" data-ce-del-org="${o.id}">Suppr.</button></div>`:''}</div></div>`}).join('')||'<p class="ce-muted">Aucune organisation.</p>'}</article><article class="card"><div class="ce-hero"><div><h3>Groupes</h3><p>Limite utilisateur + affectation initiale et budget révisé du groupe.</p></div>${ceCanEdit()?'<button id="ceAddGroup" class="btn primary">+ Groupe</button>':''}</div>${m.groups.map(g=>{const o=m.orgs.find(x=>+x.id===+g.Organisation),used=ceGroupUsed(m,g.id),alloc=Math.max(0,+g.Enveloppe_Allouee||0),max=Math.max(0,+g.Plafond_Groupe_Total||0),borrow=Math.max(0,used-alloc),ratio=alloc?used/alloc:0;return `<div class="ce-org"><b>${esc(g.Nom)}</b><div class="ce-muted">${esc(o?.Nom||'—')} · ${money(g.Limite_User_Mois,'EUR')} / user / mois</div><div style="margin-top:8px">Affectation initiale : <b>${money(alloc,'EUR')}</b> · consommé : <b class="${borrow>0?'ce-danger':''}">${money(used,'EUR')}</b> · reste vs affectation : <b>${money(alloc-used,'EUR')}</b></div><div class="ce-muted">${borrow>0?`⚠ ${money(borrow,'EUR')} consommés au-delà de l’affectation du groupe (pris sur la marge des autres groupes / de l’organisation).`:`${alloc?pct(ratio):'—'} de l’affectation initiale`} ${max?` · budget révisé ${money(max,'EUR')}`:' · aucun budget révisé défini'}</div>${ceCanEdit()?`<div style="margin-top:8px"><button class="mini-btn" data-ce-group="${g.id}">Modifier</button> <button class="mini-btn ce-danger" data-ce-del-group="${g.id}">Suppr.</button></div>`:''}</div>`}).join('')||'<p class="ce-muted">Aucun groupe.</p>'}</article></div>`;document.getElementById('ceAddOrg')?.addEventListener('click',()=>ceOrgDialog(m));document.getElementById('ceAddGroup')?.addEventListener('click',()=>ceGroupDialog(m));body.querySelectorAll('[data-ce-org]').forEach(b=>b.onclick=()=>ceOrgDialog(m,m.orgs.find(x=>+x.id===+b.dataset.ceOrg)));body.querySelectorAll('[data-ce-group]').forEach(b=>b.onclick=()=>ceGroupDialog(m,m.groups.find(x=>+x.id===+b.dataset.ceGroup)));body.querySelectorAll('[data-ce-del-org]').forEach(b=>b.onclick=()=>ceDeleteOrg(m,+b.dataset.ceDelOrg));body.querySelectorAll('[data-ce-del-group]').forEach(b=>b.onclick=()=>ceDeleteGroup(m,+b.dataset.ceDelGroup))}
