...
function domainTeamBudgetBreakdown(m,domainId){
  ...
  for(const a of allocs){
    ...
    const cur=allocByOffer.get(oid)||{budget:0,fixed:0,variable:0,licenses:0};
    cur.budget+=+a.Budget_Total_USD||0;
    cur.fixed+=+a.Cout_Abonnement||0;
    cur.variable+=+a.Cout_Overage||0;
  ...
  for(const [oid,a] of allocByOffer){
    ...
    for(const t of teams){
      ...
      const ratio=n/totalNamed;
      const share=a.budget*ratio;
      const fixedShare=a.fixed*ratio;
      const variableShare=a.variable*ratio;
      ...
      row.licenses+=n;
      row.fixed+=fixedShare;
      row.variable+=variableShare;
      row.budget+=share;
      byTeam.set(+t.id,row);
    }
  }
...
function scenarioDomainTeamBudgetHtml(m,domainId){
  ...
  return `<div class="scenario-team-budget">
    <div class="scenario-team-budget-head">
      <div>
        <span class="domain-label">${compareLabelV71("PRÉ-SIMULATION")}</span>
        <h4>${compareLabelV71("Répartition budgétaire par équipe et par offre")}</h4>
    ...
    <div class="tablewrap"><table class="scenario-team-budget-table">
      <thead><tr>
        <th>${compareLabelV71("Équipe")}</th>
        <th>${compareLabelV71("Fournisseur")}</th>
        <th>${compareLabelV71("Type d’offre")}</th>
        <th>${compareLabelV71("Licences")}</th>
        <th>${compareLabelV71("Fixe")}</th>
        <th>${compareLabelV71("Variable")}</th>
        <th>${compareLabelV71("Budget USD")}</th>
        <th>${compareLabelV71("Budget EUR")}</th>
        <th>${compareLabelV71("Part du domaine")}</th>
      </tr></thead>
      <tbody>
        ${x.offerRows.map(r=>`<tr>
          ...
          <td class="num"><b>${money(r.fixed)}</b><small class="scenario-team-eur">≈ ${money(r.fixed*(+m.rate||0),'EUR')}</small></td>
          <td class="num"><b>${money(r.variable)}</b><small class="scenario-team-eur">≈ ${money(r.variable*(+m.rate||0),'EUR')}</small></td>
        ...
        <tr class="total">
          <td colspan="3">${compareLabelV71("TOTAL RÉPARTI")}</td>
          <td class="num">${num(x.offerRows.reduce((s,r)=>s+r.licenses,0))}</td>
          <td class="num"><b>${money(x.offerRows.reduce((s,r)=>s+r.fixed,0))}</b><small class="scenario-team-eur">≈ ${money(x.offerRows.reduce((s,r)=>s+r.fixed,0)*(+m.rate||0),'EUR')}</small></td>
          <td class="num"><b>${money(x.offerRows.reduce((s,r)=>s+r.variable,0))}</b><small class="scenario-team-eur">≈ ${money(x.offerRows.reduce((s,r)=>s+r.variable,0)*(+m.rate||0),'EUR')}</small></td>
    ...
    <div class="scenario-team-totals">
      ${x.rows.map(r=>{
        const rx=roiRhComputed(m,+domainId,+r.teamId);
        const lic=+r.annualEquivalentEUR||+rx.licenseAnnual||0;
        const n1=+rx.n1.cost||0,n=+rx.n.cost||0;
        const hrSaving=n1-n,totalN=n+lic,gain=n1-totalN,roiPct=n1?gain/n1:0;
        return `<div class="scenario-team-total-card compact-v115">
          <div class="scenario-team-card-head">
            <div class="scenario-team-card-icon">👥</div>
            <div class="scenario-team-card-titlewrap">
              <span class="scenario-team-card-title">${esc(r.team)}</span>
              <small>${num(r.licenses)} licence(s) · toutes offres</small>
            </div>
          </div>

          <div class="scenario-team-budget-kpis">
            <div class="team-budget-kpi">
              <span>Fixe</span>
              <b>${money(r.fixed)}</b>
              <small>≈ ${money(r.fixed*(+m.rate||0),'EUR')}</small>
            </div>
            <div class="team-budget-kpi">
              <span>Variable</span>
              <b>${money(r.variable)}</b>
              <small>≈ ${money(r.variable*(+m.rate||0),'EUR')}</small>
            </div>
            <div class="team-budget-kpi team-budget-total">
              <span>Total</span>
              <b>${money(r.budget)}</b>
              <small>≈ ${money(r.budget*(+m.rate||0),'EUR')}</small>
            </div>
          </div>

          <div class="scenario-team-roi">
            <div class="scenario-team-roi-title">ROI de l’équipe</div>
            <div class="scenario-team-roi-grid">
              <span><small>RH N-1</small><b>${money(n1,'EUR')}</b></span>
              <span><small>RH N</small><b>${money(n,'EUR')}</b></span>
              <span class="${hrSaving>0?'positive':''}"><small>Économie RH</small><b>${money(hrSaving,'EUR')}</b></span>
              <span><small>Coût licences</small><b>${money(lic,'EUR')}</b></span>
              <span class="${gain<0?'negative':gain>0?'positive':''}"><small>Gain net annuel</small><b>${money(gain,'EUR')}</b></span>
              <span class="scenario-team-roi-primary ${roiPct<0?'negative':roiPct>0?'positive':''}"><small>ROI</small><b>${pct(roiPct)}</b></span>
            </div>
          </div>
        </div>`;
      }).join("")}
    ...
    <div class="scenario-team-annual"><div class="scenario-team-annual-head"><h4>${compareLabelV71("Coût équivalent annuel par équipe")}</h4><p>Coût d’achat des licences ramené sur 12 mois. Le ROI de chaque équipe est affiché directement dans sa carte ci-dessus.</p></div><div class="tablewrap"><table class="scenario-team-annual-table"><thead><tr><th>${compareLabelV71("Équipe")}</th><th>${compareLabelV71("Licences")}</th><th>${compareLabelV71("Coût équivalent annuel")}</th></tr></thead><tbody>${x.rows.map(r=>`<tr><td><b>${esc(r.team)}</b></td><td class="num">${num(r.licenses)}</td><td class="num"><b>${money(r.annualEquivalentEUR,'EUR')}</b></td></tr>`).join("")}</tbody></table></div></div>
...
.scenario-team-totals{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;margin-top:10px}.scenario-team-total-card{border:1px solid #dfe5ec;border-radius:10px;background:#fff;padding:10px;display:flex;flex-direction:column;gap:6px}.scenario-team-total-card>span,.scenario-team-total-card>small,.scenario-team-total-card>b{display:block}.scenario-team-total-card>small{color:#667085}.scenario-team-total-card b small{display:inline;color:#667085;font-weight:600}.scenario-team-eur{display:block!important;margin-top:2px;color:#667085;font-size:9px;font-weight:600;white-space:nowrap}.scenario-team-card-split{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:2px;padding-top:7px;border-top:1px solid #edf0f4}.scenario-team-card-split>span{font-size:9px;color:#667085;font-weight:600}.scenario-team-card-split b{display:block;margin-top:2px;color:#182230;font-size:11px}.scenario-team-card-split small{display:block!important;margin-top:1px;color:#667085;font-size:8px}.scenario-team-total-card.compact-v115{padding:12px!important;gap:10px!important;border-radius:12px!important;box-shadow:0 4px 14px rgba(16,33,62,.05)}.scenario-team-card-head{display:flex;align-items:center;gap:10px;padding-bottom:9px;border-bottom:1px solid #edf0f4}.scenario-team-card-icon{width:28px;height:28px;border-radius:9px;background:#f1f2ff;display:flex;align-items:center;justify-content:center;font-size:14px}.scenario-team-card-titlewrap{min-width:0;display:flex;flex-direction:column;gap:2px}.scenario-team-card-title{font-weight:800;font-size:13px;color:#182230;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.scenario-team-card-titlewrap small{font-size:8px;color:#667085}.scenario-team-budget-kpis{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.team-budget-kpi{background:#f8fafc;border-radius:8px;padding:7px;display:flex;flex-direction:column;gap:2px}.team-budget-kpi span{font-size:8px;color:#667085}.team-budget-kpi b{font-size:10px;color:#182230;white-space:nowrap}.team-budget-kpi small{font-size:7px;color:#667085}.team-budget-total{background:#f4f3ff}.team-budget-total b{color:#4338ca}.scenario-team-roi{margin-top:0;padding-top:9px;border-top:1px solid #edf0f4}.scenario-team-roi-title{font-size:9px;font-weight:800;color:#182230;margin-bottom:6px}.scenario-team-roi-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}.scenario-team-roi-grid>span{display:flex;flex-direction:column;gap:2px;padding:6px;border-radius:7px;background:#f8fafc;min-width:0}.scenario-team-roi-grid small{font-size:7px;color:#667085;line-height:1.2}.scenario-team-roi-grid b{font-size:9px;color:#182230;white-space:nowrap}.scenario-team-roi-primary{background:#f4f3ff!important}.scenario-team-roi-grid .positive{background:#f1fbf5}.scenario-team-roi-grid .positive b{color:#0b7a3b}.scenario-team-roi-grid .negative{background:#fff3f3}.scenario-team-roi-grid .negative b{color:#c62828}.scenario-team-annual{margin-top:14px;padding-top:12px;border-top:1px solid #dfe5ec}.scenario-team-annual-head{margin-bottom:8px}.scenario-team-annual-head h4{margin:0 0 4px;font-size:14px}.scenario-team-annual-head p{margin:0;color:#667085;font-size:10px;line-height:1.45}.scenario-team-budget-table{background:#fff}.scenario-team-budget-table .total{font-weight:800;background:#f5f7ff}.scenario-team-budget-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:10px}.scenario-team-budget-head h4{margin:3px 0;font-size:15px}.scenario-team-budget-head p{margin:0;color:#667085;font-size:10px}
...
function printSynthesisV36(){
  ...
  const summary=`<div class="print-cover"><div><h1>Synthèse FinOps IA</h1><p>${ms.length} scénario(s) sélectionné(s)</p></div><div>Édité le ${new Date().toLocaleDateString('fr-FR')}</div></div>
    ...
    <table class="summary-table"><thead><tr><th>Scénario</th><th>${compareLabelV71("Licences")}</th><th>${compareLabelV71("Fixe")}</th><th>${compareLabelV71("Variable")}</th><th>${compareLabelV71("Budget USD")}</th><th>${compareLabelV71("Budget EUR")}</th><th>Économie annuelle</th></tr></thead><tbody>${ms.map(m=>`<tr><td><b>${esc(m.s.Nom)}</b></td><td class="num">${num(m.licenses)}</td><td class="num">${money(m.fixed)}</td><td class="num">${money(m.over)}</td><td class="num"><b>${money(m.total)}</b></td><td class="num">${money(m.total*m.rate,'EUR')}</td><td class="num ${m.savingAnnual<0?'negative':''}">${money(m.savingAnnual,'EUR')}</td></tr>`).join('')}</tbody></table>
...
function presimHtmlDocumentV62(fiche){
  ...
  <section class="section"><h2>${esc(uiLabelValue("presim","Équipes"))}</h2><table><thead><tr><th>${compareLabelV71("Équipe")}</th><th>Plan par défaut</th><th>Ressources actives</th></tr></thead><tbody>${teams.filter(t=>t.Actif!==false).map(t=>`<tr><td><b>${esc(t.Nom||'')}</b></td><td>${esc(offerDisplayName(t.Offre_Defaut))}</td><td class="num">${resources.filter(r=>r.Actif!==false&&+r.Equipe===+t.id).length}</td></tr>`).join('')||'<tr><td colspan="3">Aucune équipe</td></tr>'}</tbody></table></section>
  <section class="section"><h2>${esc(uiLabelValue("presim","Ressources nominatives"))}</h2><table><thead><tr><th>Ressource</th><th>Profil</th><th>${compareLabelV71("Équipe")}</th><th>Plan individuel</th><th>Plan effectif</th><th>Commentaire</th></tr></thead><tbody>${resourceRows||'<tr><td colspan="6">Aucune ressource active</td></tr>'}</tbody></table></section>
  <section class="section"><h2>${esc(uiLabelValue("presim","Synthèse par équipe"))}</h2><table><thead><tr><th>${compareLabelV71("Équipe")}</th><th>${compareLabelV71("Fournisseur")}</th><th>Plan</th><th>${compareLabelV71("Licences")}</th></tr></thead><tbody>${teamSummary.map(x=>`<tr><td>${esc(x.team)}</td><td>${esc(x.provider)}</td><td>${esc(x.offer)}</td><td class="num"><b>${x.count}</b></td></tr>`).join('')||'<tr><td colspan="4">Aucune donnée</td></tr>'}</tbody></table></section>
  <section class="section"><h2>${esc(uiLabelValue("presim","Synthèse consolidée des licences"))}</h2><table><thead><tr><th>${compareLabelV71("Fournisseur")}</th><th>Offre</th><th>Équipes</th><th>${compareLabelV71("Licences")}</th></tr></thead><tbody>${summary.map(x=>`<tr><td>${esc(x.provider)}</td><td>${esc(x.offer)}</td><td class="num">${x.teams}</td><td class="num"><b>${x.count}</b></td></tr>`).join('')||'<tr><td colspan="4">Aucune donnée</td></tr>'}</tbody></table></section>
...
function renderPreSimulation(){
  ...
  <article class="card">
    ...
    <div class="tablewrap"><table><thead><tr><th>${compareLabelV71("Équipe")}</th><th>Service</th><th>Plan IA par défaut</th><th>Ordre</th><th>Commentaire</th><th>Actif</th><th></th></tr></thead>
  ...
  <article class="card">
    ...
    <div class="tablewrap"><table>
      <thead><tr><th>Ressource</th><th>Profil</th><th>${compareLabelV71("Équipe")}</th><th>Plan individuel</th><th>Plan effectif</th><th>Commentaire</th><th>Actif</th><th></th></tr></thead>
  ...
  <article class="card">
    ...
    <div class="tablewrap"><table><thead><tr><th>${compareLabelV71("Équipe")}</th><th>${compareLabelV71("Fournisseur")}</th><th>Plan / offre effective</th><th>${compareLabelV71("Licences")}</th></tr></thead>
  ...
  <article class="card">
    ...
    <div class="tablewrap"><table><thead><tr><th>Domaine</th><th>${compareLabelV71("Fournisseur")}</th><th>Offre</th><th>Équipes</th><th>${compareLabelV71("Licences")}</th></tr></thead>
...
const UI_LABEL_CATALOG_V65=[
  ...
  ['compare','Enregistrer le fichier HTML'],['compare','Éditeur de l’outil'],['compare','ROI par équipe du domaine'],['compare','ROI PAR ÉQUIPE'],['compare','ROI de l’équipe'],['compare','Synthèse'],['compare','Détails des scénarios'],
...
const FINOPS_ACL_RESOURCES=[
  ...
  {tableId:"Claude_Consommations",colIds:"*",kind:"global",mode:"userEdit"},
  {tableId:"Claude_Reallocations",colIds:"*",kind:"global",mode:"userEdit"},
...
function ceScoped(sid){
  ...
  const resources=ceRows('claudeResources').filter(x=>+x.Scenario===+sid&&(orgIds.has(+x.Organisation)||groupIds.has(+x.Groupe))), resourceIds=new Set(resources.map(x=>+x.id));
  const consumption=ceRows('claudeConsumption').filter(x=>+x.Scenario===+sid&&(orgIds.has(+x.Organisation)||groupIds.has(+x.Groupe)||resourceIds.has(+x.Ressource)));
  const reallocations=ceRows('claudeRealloc').filter(x=>+x.Scenario===+sid&&(orgIds.has(+x.Organisation)||groupIds.has(+x.Groupe_Demandeur)||groupIds.has(+x.Groupe_Donneur)));
  return {orgs,groups,resources,consumption,reallocations};
}
...
function cePeriodUsed(m,rid,period){return m.consumption.filter(c=>+c.Ressource===+rid&&String(c.Periode||'')===String(period||'')).reduce((a,c)=>a+Math.max(0,+c.Montant||0),0)}
function ceGroupUsed(m,gid){return m.consumption.filter(c=>+c.Groupe===+gid).reduce((a,c)=>a+Math.max(0,+c.Montant||0),0)}
function ceGroupMonths(m,g){return Math.max(1,+g?.Duree_Engagement_Mois||+m?.s?.Duree_Engagement_Mois||12)}
function cePeriodAdd(period,offset){const m=/^(\d{4})-(\d{2})$/.exec(String(period||''));if(!m)return '';const d=new Date(Date.UTC(+m[1],+m[2]-1+offset,1));return `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}`}
function ceScenarioStartPeriod(m){const periods=(m.consumption||[]).map(c=>String(c.Periode||'')).filter(x=>/^\d{4}-\d{2}$/.test(x)).sort();if(periods.length)return periods[0];const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}
function ceGroupMonthUsed(m,gid,period){return (m.consumption||[]).filter(c=>+c.Groupe===+gid&&String(c.Periode||'')===String(period)).reduce((a,c)=>a+Math.max(0,+c.Montant||0),0)}
function ceModel(sid){const s=ceRows('claudeScenarios').find(x=>+x.id===+sid),x=ceScoped(sid),active=x.resources.filter(r=>r.Actif!==false),exposure=active.reduce((a,r)=>a+ceLimit(r,x.groups),0),cap=x.orgs.reduce((a,o)=>a+Math.max(0,+o.Plafond_Global||0),0),consumed=x.consumption.reduce((a,c)=>a+Math.max(0,+c.Montant||0),0),committed=x.orgs.reduce((a,o)=>a+Math.max(0,+o.Enveloppe_Engagee||0),0),remaining=committed-consumed;return {...x,s,active,exposure,cap,margin:cap-exposure,consumed,committed,remaining}}
...
function renderClaudeEnterprise(){
  ...
  <article class="card ce-scenario"><label class="field">Scénario<select id="ceScenarioSelect">${scenarios.map(s=>`<option value="${s.id}" ${+s.id===+m.s.id?'selected':''}>${esc(s.Nom)}</option>`).join('')}</select></label><div class="ce-actions">${editable?'<button id="ceNew" class="btn secondary">+ Nouveau</button><button id="ceDuplicate" class="btn secondary">Dupliquer</button><button id="ceRename" class="btn secondary">Renommer</button><button id="ceDeleteScenario" class="btn secondary ce-danger">Supprimer</button>':''}</div><span class="ce-muted">${esc(m.s.Description||m.s.Commentaire||'')} · Engagement : ${Math.max(1,+m.s.Duree_Engagement_Mois||12)} mois</span></article>
  <div class="ce-kpis"><div class="ce-kpi"><div class="ce-muted">RESSOURCES</div><div class="v">${num(m.resources.length)}</div><div class="l">${num(m.active.length)} actives</div></div><div class="ce-kpi"><div class="ce-muted">ENVELOPPE ENGAGÉE</div><div class="v">${money(m.committed,'EUR')}</div><div class="l">prépayé ou PO</div></div><div class="ce-kpi"><div class="ce-muted">CONSOMMÉ RÉEL</div><div class="v">${money(m.consumed,'EUR')}</div><div class="l">${m.committed?pct(m.consumed/m.committed):'—'} de l’enveloppe</div></div><div class="ce-kpi"><div class="ce-muted">RESTE BUDGÉTAIRE</div><div class="v ${m.remaining<0?'ce-danger':''}">${money(m.remaining,'EUR')}</div><div class="l">${m.cap?`plafond global ${money(m.cap,'EUR')}`:'plafond global non défini'}</div></div></div>
  <div class="ce-tabs"><button class="ce-tab ${CE_TAB==='overview'?'active':''}" data-ce-tab="overview">Vue d’ensemble</button><button class="ce-tab ${CE_TAB==='resources'?'active':''}" data-ce-tab="resources">Ressources</button><button class="ce-tab ${CE_TAB==='structure'?'active':''}" data-ce-tab="structure">Organisations & groupes</button><button class="ce-tab ${CE_TAB==='consumption'?'active':''}" data-ce-tab="consumption">Consommation</button></div><div id="ceTabBody"></div>`;
  ...
  if(CE_TAB==='resources')ceRenderResources(m);else if(CE_TAB==='structure')ceRenderStructure(m);else if(CE_TAB==='consumption')ceRenderConsumption(m);else ceRenderOverview(m,pctCap);
}
function ceRenderOverview(m,pctCap){
  ...
  const info=(label,text)=>`${label} <span class="ce-info-btn" role="button" tabindex="0" data-ce-info="${encodeURIComponent(JSON.stringify({label,text}))}" aria-label="Afficher l’explication de ${esc(label)}">i</span>`;
  ...
  const addMonthsDate=(period,n)=>{const m=/^(\d{4})-(\d{2})$/.exec(period);if(!m)return '—';const d=new Date(Date.UTC(+m[1],+m[2]-1+Math.max(0,Math.ceil(n)-1),1));return d.toLocaleDateString('fr-FR',{month:'short',year:'numeric',timeZone:'UTC'})};
  const rows=m.groups.map(g=>{
    const o=m.orgs.find(x=>+x.id===+g.Organisation);
    const allocated=Math.max(0,+g.Enveloppe_Allouee||0);
    const currentCap=Math.max(0,+g.Plafond_Groupe_Total||0);
    const months=ceGroupMonths(m,g), elapsed=elapsedFor(months);
    const monthlyBudget=allocated/months, revisedMonthly=currentCap/months;
    const used=ceGroupUsed(m,g.id);
    const incoming=accepted.filter(r=>+r.Groupe_Demandeur===+g.id).reduce((a,r)=>a+Math.max(0,+r.Montant||0),0);
    const outgoing=accepted.filter(r=>+r.Groupe_Donneur===+g.id).reduce((a,r)=>a+Math.max(0,+r.Montant||0),0);
    const reallocNet=incoming-outgoing, remaining=currentCap-used;
    const theoreticalToDate=revisedMonthly*elapsed;
    const burnRate=theoreticalToDate>0?used/theoreticalToDate:0;
    const avgMonthly=used/elapsed;
    const landing=avgMonthly*months;
    const need=Math.max(0,landing-currentCap);
    const releasable=Math.max(0,currentCap-landing);
    const exhaustMonths=avgMonthly>0?currentCap/avgMonthly:Infinity;
    const exhaustDate=isFinite(exhaustMonths)?addMonthsDate(startPeriod,exhaustMonths):'—';
    const state=need>0||burnRate>=1.15?'danger':burnRate>=.95?'warn':'ok';
    let cumulativeUsed=0; const monthly=[];
    for(let i=0;i<months;i++){
      const period=cePeriodAdd(startPeriod,i),monthUsed=ceGroupMonthUsed(m,g.id,period);
      cumulativeUsed+=monthUsed;
      const cumulativeBudget=Math.min(allocated,monthlyBudget*(i+1));
      monthly.push({period,monthUsed,cumulativeUsed,cumulativeBudget,gap:cumulativeUsed-cumulativeBudget});
    }
    return {g,o,allocated,currentCap,months,elapsed,monthlyBudget,used,incoming,outgoing,reallocNet,remaining,burnRate,avgMonthly,landing,need,releasable,exhaustDate,state,monthly};
  });
  const allocatedTotal=rows.reduce((a,x)=>a+x.allocated,0),currentCaps=rows.reduce((a,x)=>a+x.currentCap,0),usedTotal=rows.reduce((a,x)=>a+x.used,0),remainingGlobal=m.cap-usedTotal;
  const monthlyEstimate=rows.reduce((a,x)=>a+x.monthlyBudget,0), landingTotal=rows.reduce((a,x)=>a+x.landing,0);
  const globalElapsed=Math.max(1,...rows.map(x=>x.elapsed)), globalMonthlyTarget=currentCaps/Math.max(1,...rows.map(x=>x.months));
  const globalBurn=globalMonthlyTarget>0?(usedTotal/globalElapsed)/globalMonthlyTarget:0;
  const globalAvg=usedTotal/globalElapsed, globalExhaust=globalAvg>0?addMonthsDate(startPeriod,m.cap/globalAvg):'—';
  const riskRows=rows.filter(x=>x.state!=='ok').length;
  const maxMonths=Math.max(0,...rows.map(x=>x.months)),periods=Array.from({length:maxMonths},(_,i)=>cePeriodAdd(startPeriod,i));
  const monthlyHeaders=periods.map(p=>`<th>${esc(p)}<br><span class="ce-muted">écart cumulé</span></th>`).join('');
  const monthlyRows=rows.map(x=>`<tr><td><b>${esc(x.g.Nom)}</b><br><span class="ce-muted">${x.months} mois · ${money(x.monthlyBudget,'EUR')}/mois</span></td><td class="num">${money(x.allocated,'EUR')}</td>${periods.map((p,i)=>{if(i>=x.months)return '<td class="num ce-muted">—</td>';const q=x.monthly[i],cls=q.gap>0?'ce-danger':'';return `<td class="num ${cls}"><b>${q.gap>=0?'+':''}${money(q.gap,'EUR')}</b><br><span class="ce-muted">Conso ${money(q.cumulativeUsed,'EUR')} / cible ${money(q.cumulativeBudget,'EUR')}</span></td>`}).join('')}</tr>`).join('');
  const needs=rows.filter(x=>x.need>0).sort((a,b)=>b.need-a.need), donors=rows.filter(x=>x.releasable>0).sort((a,b)=>b.releasable-a.releasable);
  const totalNeed=needs.reduce((a,x)=>a+x.need,0), totalRelease=donors.reduce((a,x)=>a+x.releasable,0), residual=Math.max(0,totalNeed-totalRelease);
  const recHtml=totalNeed?`<section class="ce-reco ce-reco-modern"><div class="ce-reco-head"><div><span class="ce-section-kicker">AIDE À LA DÉCISION</span><h4>Recommandations FinOps</h4><p>Analyse des besoins et des capacités de réallocation sur la base de l’atterrissage actuel.</p></div>${ceCanEdit()?'<button type="button" id="cePrepareRealloc" class="ce-action-btn primary"><span>↗</span> Préparer une réallocation</button>':''}</div><div class="ce-reco-cards"><div class="ce-reco-card danger"><span class="ce-reco-icon">↗</span><div><span class="ce-reco-label">Besoin prévisionnel total</span><strong>${money(totalNeed,'EUR')}</strong><small>${needs.length} groupe(s) en dépassement projeté</small></div></div><div class="ce-reco-card success"><span class="ce-reco-icon">↘</span><div><span class="ce-reco-label">Capacité potentielle de réallocation</span><strong>${money(Math.min(totalNeed,totalRelease),'EUR')}</strong><small>${donors.length} groupe(s) avec marge projetée</small></div></div><div class="ce-reco-card ${residual?'warn':'info'}"><span class="ce-reco-icon">◎</span><div><span class="ce-reco-label">Besoin résiduel</span><strong>${money(residual,'EUR')}</strong><small>${residual?'Après réallocation potentielle':'Besoin théoriquement absorbable'}</small></div></div></div><div class="ce-reco-detail"><div>${needs.map(x=>`<span><b>${esc(x.g.Nom)}</b> : +${money(x.need,'EUR')}</span>`).join('')}</div><div>${donors.map(x=>`<span><b>${esc(x.g.Nom)}</b> : ${money(x.releasable,'EUR')} libérables</span>`).join('')||'<span>Aucune capacité libérable détectée.</span>'}</div></div></section>`:`<section class="ce-reco ce-reco-modern"><div class="ce-reco-head"><div><span class="ce-section-kicker">AIDE À LA DÉCISION</span><h4>Recommandations FinOps</h4><p>Aucun dépassement prévisionnel détecté au rythme actuel.</p></div></div><div class="ce-reco-empty">✓ Les budgets révisés couvrent les atterrissages projetés.</div></section>`;
  body.innerHTML=`<div class="ce-overview-modern">
  <section class="ce-overview-head">
    <div><span class="ce-section-kicker">CLAUDE ENTERPRISE</span><h2>Vue d’ensemble Claude Enterprise</h2><p>Suivi de la consommation, prévisions et recommandations FinOps.</p></div>
  </section>
  <section class="ce-kpi-grid">
    <div class="ce-kpi-card"><span class="ce-kpi-icon blue">€</span><div><span class="ce-kpi-label">Plafond global</span><strong>${money(m.cap,'EUR')}</strong><small>Plafond organisationnel bloquant</small></div></div>
    <div class="ce-kpi-card"><span class="ce-kpi-icon orange">▥</span><div><span class="ce-kpi-label">Consommé</span><strong>${money(usedTotal,'EUR')}</strong><small>${m.cap?pct(usedTotal/m.cap):'—'} du plafond</small></div></div>
    <div class="ce-kpi-card"><span class="ce-kpi-icon green">◔</span><div><span class="ce-kpi-label">Reste</span><strong>${money(remainingGlobal,'EUR')}</strong><small>${m.cap?pct(Math.max(0,remainingGlobal)/m.cap):'—'} du plafond</small></div></div>
    <div class="ce-kpi-card"><span class="ce-kpi-icon red">↗</span><div><span class="ce-kpi-label">${info('Burn rate global',`Compare le rythme réel au rythme du budget révisé. 100 % = rythme conforme ; 120 % = consommation 20 % trop rapide. Calcul actuel : moyenne consommée ${money(globalAvg,'EUR')}/mois ÷ cible ${money(globalMonthlyTarget,'EUR')}/mois.`)}</span><strong class="${globalBurn>1.15?'danger-text':''}">${pct(globalBurn)}</strong><small>${globalBurn>1?`+${Math.round((globalBurn-1)*100)} % vs rythme prévu`:'Rythme conforme ou inférieur'}</small></div></div>
    <div class="ce-kpi-card"><span class="ce-kpi-icon violet">⌁</span><div><span class="ce-kpi-label">${info('Atterrissage',`Estimation de la consommation totale en fin d’engagement si le rythme moyen observé se poursuit.`)}</span><strong class="${landingTotal>currentCaps?'danger-text':''}">${money(landingTotal,'EUR')}</strong><small>${landingTotal>currentCaps?`+${money(landingTotal-currentCaps,'EUR')} vs budgets révisés`:'Sous les budgets révisés'}</small></div></div>
    <div class="ce-kpi-card"><span class="ce-kpi-icon blue">◫</span><div><span class="ce-kpi-label">${info('Épuisement estimé',`Date à laquelle le plafond global serait épuisé si le rythme moyen de consommation observé se maintient.`)}</span><strong>${globalExhaust}</strong><small>${riskRows} groupe(s) à surveiller</small></div></div>
  </section>
  <section class="ce-pilot-card">
    <div class="ce-pilot-head"><div><h3>Pilotage par groupe</h3><p>Suivi budgétaire, rythme de consommation et prévisions à fin d’engagement.</p></div></div>
    <div class="ce-progress-line"><span style="width:${Math.min(100,m.cap?usedTotal/m.cap*100:0)}%"></span></div>
    <div class="ce-progress-caption">${m.cap?pct(usedTotal/m.cap):'—'} du plafond global consommé · reste ${money(remainingGlobal,'EUR')}</div>
    <div class="ce-modern-tablewrap"><table class="ce-modern-table"><thead><tr><th>Groupe</th><th>Affectation<br>initiale</th><th>Budget<br>révisé</th><th>${info('Budget mensuel','Affectation initiale ÷ durée d’engagement du groupe.')}</th><th>Consommé</th><th>${info('Burn rate','Consommation réelle cumulée ÷ budget révisé théorique cumulé à date. 100 % = rythme conforme.')}</th><th>${info('Atterrissage','Moyenne mensuelle réellement consommée × durée d’engagement.')}</th><th>${info('Besoin / marge','Besoin = atterrissage − budget révisé. Marge = budget révisé − atterrissage.')}</th><th>${info('Épuisement','Date estimée d’épuisement du budget révisé au rythme actuel.')}</th><th>Statut</th></tr></thead><tbody>
    ${rows.map(x=>`<tr><td class="ce-group-name"><b>${esc(x.g.Nom)}</b><small>${esc(x.o?.Nom||'—')} · ${x.elapsed}/${x.months} mois</small></td><td>${money(x.allocated,'EUR')}</td><td><b>${money(x.currentCap,'EUR')}</b><small>${x.reallocNet>=0?'+':''}${money(x.reallocNet,'EUR')} réalloué</small></td><td>${money(x.monthlyBudget,'EUR')}</td><td>${money(x.used,'EUR')}</td><td class="${x.burnRate>1.15?'danger-text':''}"><b>${pct(x.burnRate)}</b><small>${money(x.avgMonthly,'EUR')}/mois</small></td><td class="${x.landing>x.currentCap?'danger-text':''}"><b>${money(x.landing,'EUR')}</b></td><td>${x.need>0?`<b class="danger-text">+${money(x.need,'EUR')}</b><small>besoin</small>`:`<b class="success-text">−${money(x.releasable,'EUR')}</b><small>marge</small>`}</td><td>${x.exhaustDate}</td><td>${x.state==='danger'?'<span class="ce-status danger">Critique</span>':x.state==='warn'?'<span class="ce-status warn">À surveiller</span>':'<span class="ce-status ok">Sain</span>'}</td></tr>`).join('')||'<tr><td colspan="10">Aucun groupe.</td></tr>'}
    </tbody></table></div>
    ${recHtml}
  </section>
  <section class="ce-pilot-card ce-monthly-card"><div class="ce-pilot-head"><div><h3>${info('Écart cumulé à fin de mois','Consommation réelle cumulée − budget initial théorique cumulé. Positif = consommation en avance sur la trajectoire prévue ; négatif = sous-consommation.')}</h3><p>Écart de consommation par rapport à la trajectoire budgétaire initiale.</p></div></div><div class="ce-modern-tablewrap"><table class="ce-modern-table ce-monthly-table"><thead><tr><th>Groupe</th><th>Affectation initiale</th>${monthlyHeaders}</tr></thead><tbody>${monthlyRows||`<tr><td colspan="${2+periods.length}">Aucun groupe.</td></tr>`}</tbody></table></div></section>
  <section class="ce-pilot-card ce-architecture-card"><div class="ce-pilot-head"><div><h3>Architecture budgétaire</h3><p>Répartition par organisation et groupe.</p></div></div><div class="ce-architecture-grid">${m.orgs.map(o=>{const gs=m.groups.filter(g=>+g.Organisation===+o.id),used=gs.reduce((a,g)=>a+ceGroupUsed(m,g.id),0);return `<div class="ce-org-modern"><div class="ce-org-modern-head"><b>${esc(o.Nom)}</b><span>${money(o.Plafond_Global,'EUR')}</span></div><small>Consommé ${money(used,'EUR')} · reste ${money((+o.Plafond_Global||0)-used,'EUR')}</small><div class="ce-org-groups">${gs.map(g=>{const rs=m.active.filter(r=>+r.Groupe===+g.id),ex=rs.reduce((a,r)=>a+ceLimit(r,m.groups),0);return `<div><b>${esc(g.Nom)}</b><span>${rs.length} ressource(s) · ${money(g.Limite_User_Mois,'EUR')}/user/mois</span><small>Exposition mensuelle ${money(ex,'EUR')}</small></div>`}).join('')||'<span>Aucun groupe</span>'}</div></div>`}).join('')||'<div>Aucune organisation.</div>'}</div></section>
  </div>`;
  document.getElementById('cePrepareRealloc')?.addEventListener('click',()=>ceReallocationDialog(m));
  body.querySelectorAll('[data-ce-info]').forEach(btn=>btn.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();btn.click();}}));
  body.querySelectorAll('[data-ce-info]').forEach(btn=>btn.addEventListener('click',()=>{
    let raw={label:'Information',text:''};
    try{raw=JSON.parse(decodeURIComponent(btn.dataset.ceInfo||''))}catch(_e){}
    const first=rows[0];
    let definition=raw.text||'', formula='', example='', note='';
    if(raw.label==='Budget mensuel' || raw.label==='Budget mensuel estimé'){
      definition='Montant théorique prévu chaque mois pour lisser l’affectation initiale sur la durée d’engagement du groupe.';
      formula='Affectation initiale ÷ Durée d’engagement (mois)';
      if(first) example=`<b>${esc(first.g.Nom)}</b><div class="ce-info-example-grid"><span>Affectation initiale</span><b>${money(first.allocated,'EUR')}</b><span>Durée d’engagement</span><b>${first.months} mois</b><span>Calcul</span><b>${money(first.allocated,'EUR')} ÷ ${first.months} = ${money(first.monthlyBudget,'EUR')} / mois</b></div>`;
      note='Ce montant sert de trajectoire budgétaire de référence pour comparer la consommation réelle au rythme prévu.';
    }else if(raw.label==='Burn rate' || raw.label==='Burn rate global'){
      definition='Indique si la consommation réelle progresse plus vite ou plus lentement que le rythme budgétaire prévu. 100 % = rythme conforme.';
      formula='Consommation réelle cumulée ÷ Budget théorique cumulé à date';
      if(first) example=`<b>${esc(first.g.Nom)}</b><div class="ce-info-example-grid"><span>Consommé</span><b>${money(first.used,'EUR')}</b><span>Rythme théorique</span><b>${money((first.currentCap/first.months)*first.elapsed,'EUR')}</b><span>Burn rate</span><b>${pct(first.burnRate)}</b></div>`;
      note='Au-dessus de 100 %, le groupe consomme plus vite que prévu. À 120 %, il consomme environ 20 % trop vite.';
    }else if(raw.label==='Atterrissage'){
      definition='Estimation de la consommation totale à la fin de l’engagement si le rythme observé se poursuit.';
      formula='Consommation moyenne mensuelle observée × Durée d’engagement';
      if(first) example=`<b>${esc(first.g.Nom)}</b><div class="ce-info-example-grid"><span>Moyenne observée</span><b>${money(first.avgMonthly,'EUR')} / mois</b><span>Durée</span><b>${first.months} mois</b><span>Atterrissage</span><b>${money(first.landing,'EUR')}</b></div>`;
      note='L’atterrissage permet d’anticiper un besoin de réallocation avant que le budget ne soit réellement dépassé.';
    }else if(raw.label==='Besoin / marge'){
      definition='Compare l’atterrissage prévisionnel au budget révisé actuellement autorisé pour le groupe.';
      formula='Besoin = max(0, Atterrissage − Budget révisé) · Marge = max(0, Budget révisé − Atterrissage)';
      if(first) example=`<b>${esc(first.g.Nom)}</b><div class="ce-info-example-grid"><span>Budget révisé</span><b>${money(first.currentCap,'EUR')}</b><span>Atterrissage</span><b>${money(first.landing,'EUR')}</b><span>Résultat</span><b>${first.need>0?'Besoin '+money(first.need,'EUR'):'Marge '+money(first.releasable,'EUR')}</b></div>`;
      note='Une marge peut devenir une capacité potentiellement réallouable vers un autre groupe.';
    }else if(raw.label==='Épuisement' || raw.label==='Date estimée d’épuisement'){
      definition='Date théorique à laquelle le budget disponible serait entièrement consommé si le rythme moyen actuel se maintient.';
      formula='Budget disponible ÷ Consommation moyenne mensuelle, reporté depuis le début de période';
      if(first) example=`<b>${esc(first.g.Nom)}</b><div class="ce-info-example-grid"><span>Budget révisé</span><b>${money(first.currentCap,'EUR')}</b><span>Moyenne observée</span><b>${money(first.avgMonthly,'EUR')} / mois</b><span>Épuisement estimé</span><b>${first.exhaustDate}</b></div>`;
      note='Cette date est une projection : elle évolue à chaque nouvelle consommation enregistrée.';
    }else if(raw.label==='Écart cumulé à fin de mois'){
      definition='Mesure, à la fin de chaque mois, l’avance ou le retard de consommation par rapport à la trajectoire budgétaire initiale.';
      formula='Consommation réelle cumulée − Budget théorique cumulé';
      note='Un écart positif signifie que le groupe consomme en avance sur le budget prévu ; un écart négatif indique une sous-consommation.';
    }
    ceInfoDialog(raw.label,definition,formula,example,note);
  }));

}
function ceRenderResources(m){
  const body=document.getElementById('ceTabBody');
  const rows=m.resources.map(r=>{const g=m.groups.find(x=>+x.id===+r.Groupe),o=m.orgs.find(x=>+x.id===+r.Organisation),limit=ceLimit(r,m.groups),origin=r.Limite_Individuelle_Active?'Dérogation individuelle':'Héritée du groupe';return {r,g,o,limit,origin}});
  body.innerHTML=`<article class="card"><div class="ce-hero"><div><h3>Ressources</h3><p>Les utilisateurs héritent automatiquement de la limite mensuelle de leur groupe, sauf dérogation individuelle.</p></div>${ceCanEdit()?'<button id="ceAddResource" class="btn primary">+ Ressource</button>':''}</div><div class="tablewrap"><table><thead><tr><th>Utilisateur</th><th>Organisation</th><th>Groupe</th><th>Limite groupe</th><th>Limite effective</th><th>Origine</th><th>Statut</th><th></th></tr></thead><tbody>${rows.map(x=>`<tr><td><b>${esc(x.r.Nom)}</b><br><span class="ce-muted">${esc(x.r.Email||'')}</span></td><td>${esc(x.o?.Nom||'—')}</td><td>${esc(x.g?.Nom||'—')}</td><td class="num">${money(x.g?.Limite_User_Mois||0,'EUR')}/mois</td><td class="num"><b>${money(x.limit,'EUR')}/mois</b></td><td>${x.r.Limite_Individuelle_Active?'<span class="ce-origin override">Dérogation</span>':'<span class="ce-origin">Groupe</span>'}</td><td>${x.r.Actif===false?'<span class="badge">Inactif</span>':'<span class="badge ok">Actif</span>'}</td><td>${ceCanEdit()?`<button class="mini-btn" data-ce-resource="${x.r.id}">Modifier</button> <button class="mini-btn ce-danger" data-ce-del-resource="${x.r.id}">Suppr.</button>`:''}</td></tr>`).join('')||'<tr><td colspan="8">Aucune ressource.</td></tr>'}</tbody></table></div><p class="ce-muted" style="margin-top:10px">Une modification de la limite du groupe se répercute immédiatement sur la <b>limite effective</b> de tous les utilisateurs du groupe sans dérogation. La colonne technique de dérogation individuelle reste inchangée tant qu’aucune exception n’est activée.</p></article>`;
  document.getElementById('ceAddResource')?.addEventListener('click',()=>ceResourceDialog(m));
  body.querySelectorAll('[data-ce-resource]').forEach(b=>b.onclick=()=>ceResourceDialog(m,m.resources.find(x=>+x.id===+b.dataset.ceResource)));
  body.querySelectorAll('[data-ce-del-resource]').forEach(b=>b.onclick=()=>ceDeleteRecord(T.claudeResources,+b.dataset.ceDelResource,'ressource'));
}
function ceRenderStructure(m){const body=document.getElementById('ceTabBody');body.innerHTML=`<div class="ce-layout"><article class="card"><div class="ce-hero"><div><h3>Organisations</h3><p>Plafonds globaux du scénario.</p></div>${ceCanEdit()?'<button id="ceAddOrg" class="btn primary">+ Organisation</button>':''}</div>${m.orgs.map(o=>{const gs=m.groups.filter(g=>+g.Organisation===+o.id),allocated=gs.reduce((a,g)=>a+Math.max(0,+g.Enveloppe_Allouee||0),0),used=gs.reduce((a,g)=>a+ceGroupUsed(m,g.id),0);return `<div class="ce-org"><div style="display:flex;justify-content:space-between;gap:8px"><div><b>${esc(o.Nom)}</b><div class="ce-muted">${money(o.Plafond_Global,'EUR')} global · ${esc(o.Mode_Facturation||'PO')} · enveloppe ${money(o.Enveloppe_Engagee,'EUR')}</div><div class="ce-muted">Réparti aux groupes : <b>${money(allocated,'EUR')}</b> · non affecté : <b>${money(Math.max(0,(+o.Enveloppe_Engagee||0)-allocated),'EUR')}</b> · consommé : <b>${money(used,'EUR')}</b></div></div>${ceCanEdit()?`<div><button class="mini-btn" data-ce-org="${o.id}">Modifier</button> <button class="mini-btn ce-danger" data-ce-del-org="${o.id}">Suppr.</button></div>`:''}</div></div>`}).join('')||'<p class="ce-muted">Aucune organisation.</p>'}</article><article class="card"><div class="ce-hero"><div><h3>Groupes</h3><p>Limite utilisateur + affectation initiale et budget révisé du groupe.</p></div>${ceCanEdit()?'<button id="ceAddGroup" class="btn primary">+ Groupe</button>':''}</div>${m.groups.map(g=>{const o=m.orgs.find(x=>+x.id===+g.Organisation),used=ceGroupUsed(m,g.id),alloc=Math.max(0,+g.Enveloppe_Allouee||0),max=Math.max(0,+g.Plafond_Groupe_Total||0),borrow=Math.max(0,used-alloc),ratio=alloc?used/alloc:0;return `<div class="ce-org"><b>${esc(g.Nom)}</b><div class="ce-muted">${esc(o?.Nom||'—')} · ${money(g.Limite_User_Mois,'EUR')} / user / mois</div><div style="margin-top:8px">Affectation initiale : <b>${money(alloc,'EUR')}</b> · consommé : <b class="${borrow>0?'ce-danger':''}">${money(used,'EUR')}</b> · reste vs affectation : <b>${money(alloc-used,'EUR')}</b></div><div class="ce-muted">${borrow>0?`⚠ ${money(borrow,'EUR')} consommés au-delà de l’affectation du groupe (pris sur la marge des autres groupes / de l’organisation).`:`${alloc?pct(ratio):'—'} de l’affectation initiale`} ${max?` · budget révisé ${money(max,'EUR')}`:' · aucun budget révisé défini'}</div>${ceCanEdit()?`<div style="margin-top:8px"><button class="mini-btn" data-ce-group="${g.id}">Modifier</button> <button class="mini-btn ce-danger" data-ce-del-group="${g.id}">Suppr.</button></div>`:''}</div>`}).join('')||'<p class="ce-muted">Aucun groupe.</p>'}</article></div>`;document.getElementById('ceAddOrg')?.addEventListener('click',()=>ceOrgDialog(m));document.getElementById('ceAddGroup')?.addEventListener('click',()=>ceGroupDialog(m));body.querySelectorAll('[data-ce-org]').forEach(b=>b.onclick=()=>ceOrgDialog(m,m.orgs.find(x=>+x.id===+b.dataset.ceOrg)));body.querySelectorAll('[data-ce-group]').forEach(b=>b.onclick=()=>ceGroupDialog(m,m.groups.find(x=>+x.id===+b.dataset.ceGroup)));body.querySelectorAll('[data-ce-del-org]').forEach(b=>b.onclick=()=>ceDeleteOrg(m,+b.dataset.ceDelOrg));body.querySelectorAll('[data-ce-del-group]').forEach(b=>b.onclick=()=>ceDeleteGroup(m,+b.dataset.ceDelGroup))}
function ceReallocStatusLabel(st){return st==='ACCEPTEE'?'Acceptée':st==='REFUSEE'?'Refusée':'En attente'}
function ceGroupEffectiveCap(m,gid){const g=m.groups.find(x=>+x.id===+gid);if(!g)return 0;return Math.max(0,+g.Plafond_Groupe_Total||0)}
async function ceAcceptReallocation(m,rq){if(!ceCanEdit())return;const from=m.groups.find(g=>+g.id===+rq.Groupe_Donneur),to=m.groups.find(g=>+g.id===+rq.Groupe_Demandeur),amt=Math.max(0,+rq.Montant||0);if(!from||!to)throw Error('Groupe donneur ou demandeur introuvable.');if(+from.Organisation!==+to.Organisation)throw Error('La réallocation doit rester dans la même organisation.');const fromCap=Math.max(0,+from.Plafond_Groupe_Total||0),fromAlloc=Math.max(0,+from.Enveloppe_Allouee||0);if(fromCap-amt<fromAlloc-0.00001)throw Error(`Le groupe donneur ne peut pas céder ${money(amt,'EUR')} : son budget révisé tomberait sous son affectation (${money(fromAlloc,'EUR')}).`);const toCap=Math.max(0,+to.Plafond_Groupe_Total||0);await grist.docApi.applyUserActions([['UpdateRecord',T.claudeGroups,from.id,{Plafond_Groupe_Total:fromCap-amt}],['UpdateRecord',T.claudeGroups,to.id,{Plafond_Groupe_Total:toCap+amt}],['UpdateRecord',T.claudeRealloc,rq.id,{Statut:'ACCEPTEE',Date_Decision:new Date().toISOString(),Commentaire_Decision:'Réallocation acceptée'}]]);toast('Réallocation acceptée : plafonds virtuels mis à jour.');await ceOpenScenario(m.s.id,'consumption')}
async function ceRejectReallocation(m,rq){if(!ceCanEdit())return;await grist.docApi.applyUserActions([['UpdateRecord',T.claudeRealloc,rq.id,{Statut:'REFUSEE',Date_Decision:new Date().toISOString(),Commentaire_Decision:'Réallocation refusée'}]]);toast('Demande refusée.');await ceOpenScenario(m.s.id,'consumption')}
function ceReallocationDialog(m){if(m.groups.length<2){toast('Il faut au moins deux groupes.',true);return}ceDialog('Demande exceptionnelle de surconsommation',`<div class="ce-formgrid"><label class="field">Groupe demandeur<select id="ceFRTo">${m.groups.map(g=>`<option value="${g.id}">${esc(g.Nom)}</option>`).join('')}</select></label><label class="field">Groupe donneur<select id="ceFRFrom">${m.groups.map(g=>`<option value="${g.id}">${esc(g.Nom)}</option>`).join('')}</select></label><label class="field">Montant demandé (€)<input id="ceFRAmount" type="number" min="0.01" step="0.01" value="0"></label><label class="field full">Motif<input id="ceFRReason" value="" placeholder="Surconsommation exceptionnelle, projet prioritaire..."></label><div class="field full ce-muted">La demande ne modifie aucun plafond tant qu’elle n’est pas acceptée. À l’acceptation, le budget révisé du groupe donneur diminue et celui du groupe demandeur augmente du même montant. Les affectations initiales restent inchangées.</div></div>`,async d=>{const to=+d.querySelector('#ceFRTo').value,from=+d.querySelector('#ceFRFrom').value,amt=+d.querySelector('#ceFRAmount').value||0;if(to===from)throw Error('Le groupe demandeur et le groupe donneur doivent être différents.');if(amt<=0)throw Error('Le montant doit être supérieur à 0.');const gt=m.groups.find(g=>+g.id===to),gf=m.groups.find(g=>+g.id===from);if(!gt||!gf)throw Error('Groupe introuvable.');if(+gt.Organisation!==+gf.Organisation)throw Error('Les deux groupes doivent appartenir à la même organisation.');await grist.docApi.applyUserActions([['AddRecord',T.claudeRealloc,null,{Scenario:m.s.id,Organisation:+gt.Organisation||0,Groupe_Demandeur:to,Groupe_Donneur:from,Montant:amt,Motif:d.querySelector('#ceFRReason').value.trim(),Statut:'EN_ATTENTE',Date_Demande:new Date().toISOString(),Date_Decision:'',Commentaire_Decision:''}]])})}
function ceRenderConsumption(m){const body=document.getElementById('ceTabBody'),byRes=new Map();for(const c of m.consumption){const k=+c.Ressource||0;byRes.set(k,(byRes.get(k)||0)+Math.max(0,+c.Montant||0))}const rows=m.resources.map(r=>{const used=byRes.get(+r.id)||0,limit=ceLimit(r,m.groups),g=m.groups.find(x=>+x.id===+r.Groupe),o=m.orgs.find(x=>+x.id===+r.Organisation);return {r,used,limit,g,o}}),groupRows=m.groups.map(g=>{const o=m.orgs.find(x=>+x.id===+g.Organisation),siblings=m.groups.filter(x=>+x.Organisation===+g.Organisation),used=ceGroupUsed(m,g.id),allocated=Math.max(0,+g.Enveloppe_Allouee||0),cap=Math.max(0,+g.Plafond_Groupe_Total||0),orgCommitted=Math.max(0,+o?.Enveloppe_Engagee||0),orgAllocated=siblings.reduce((a,x)=>a+Math.max(0,+x.Enveloppe_Allouee||0),0),unallocated=Math.max(0,orgCommitted-orgAllocated),otherRemaining=siblings.filter(x=>+x.id!==+g.id).reduce((a,x)=>a+Math.max(0,(+x.Enveloppe_Allouee||0)-ceGroupUsed(m,x.id)),0),actualOver=Math.max(0,used-allocated),simulatedOver=Math.max(0,(cap||allocated)-allocated),potentialNeed=Math.max(actualOver,simulatedOver),onSharedReserve=Math.min(potentialNeed,unallocated),onOtherGroups=Math.max(0,potentialNeed-unallocated),remaining=allocated-used,state=used>(cap||Infinity)?'stress':onOtherGroups>0?'other':potentialNeed>0?'reserve':'ok';return {g,o,used,allocated,cap,remaining,actualOver,simulatedOver,potentialNeed,unallocated,otherRemaining,onSharedReserve,onOtherGroups,state}});body.innerHTML=`<article class="card"><div class="ce-hero"><div><h3>Consommation réelle</h3><p>Les plafonds utilisateurs restent non mutualisés. Le budget révisé d’un groupe est une hypothèse de consommation maximale : la partie au-delà de son affectation mesure le risque potentiel sur la réserve commune puis sur les autres groupes.</p></div>${ceCanEdit()?'<div class="ce-actions"><button id="ceAddConsumption" class="btn primary">+ Saisir une consommation</button><button id="ceAddReallocation" class="btn secondary">+ Demande de réallocation</button></div>':''}</div><div class="ce-kpis"><div class="ce-kpi"><div class="ce-muted">ENVELOPPE</div><div class="v">${money(m.committed,'EUR')}</div><div class="l">prépayé ou montant PO</div></div><div class="ce-kpi"><div class="ce-muted">CONSOMMÉ</div><div class="v">${money(m.consumed,'EUR')}</div><div class="l">réel cumulé</div></div><div class="ce-kpi"><div class="ce-muted">RESTE</div><div class="v ${m.remaining<0?'ce-danger':''}">${money(m.remaining,'EUR')}</div><div class="l">solde / reste budgétaire</div></div><div class="ce-kpi"><div class="ce-muted">TAUX</div><div class="v">${m.committed?pct(m.consumed/m.committed):'—'}</div><div class="l">de l’enveloppe engagée</div></div></div><h4>Vue virtuelle par groupe</h4><div class="tablewrap"><table><thead><tr><th>Groupe</th><th>Affectation</th><th>Budget révisé</th><th>Écart budget révisé &gt; affectation initiale</th><th>Consommé réel</th><th>Réserve org mobilisable</th><th>Risque sur autres groupes</th><th>Marge restante autres groupes</th><th>État</th></tr></thead><tbody>${groupRows.map(x=>`<tr><td><b>${esc(x.g.Nom)}</b><br><span class="ce-muted">${esc(x.o?.Nom||'—')}</span></td><td class="num">${money(x.allocated,'EUR')}</td><td class="num">${x.cap?money(x.cap,'EUR'):'—'}</td><td class="num ${x.simulatedOver>0?'ce-danger':''}">${money(x.simulatedOver,'EUR')}</td><td class="num"><b>${money(x.used,'EUR')}</b>${x.actualOver>0?`<br><span class="ce-danger">+${money(x.actualOver,'EUR')} vs affectation</span>`:''}</td><td class="num">${money(x.onSharedReserve,'EUR')}<br><span class="ce-muted">sur ${money(x.unallocated,'EUR')} non affectés</span></td><td class="num ${x.onOtherGroups>0?'ce-danger':''}"><b>${money(x.onOtherGroups,'EUR')}</b></td><td class="num">${money(x.otherRemaining,'EUR')}</td><td>${x.state==='stress'?'<span class="badge danger">Conso réelle > budget révisé</span>':x.state==='other'?'<span class="badge danger">Risque sur autres groupes</span>':x.state==='reserve'?'<span class="badge warn">Mobilise la réserve org</span>':'<span class="badge ok">Dans l’affectation</span>'}</td></tr>`).join('')||'<tr><td colspan="9">Aucun groupe.</td></tr>'}</tbody></table></div><p class="ce-muted" style="margin-top:8px">Lecture : si le budget révisé dépasse l’affectation du groupe, l’écart est un besoin potentiel supplémentaire. Il est d’abord comparé au montant non affecté de l’organisation ; seul le reliquat est considéré comme un risque potentiel sur les enveloppes des autres groupes.</p><h4 style="margin-top:20px">Réallocations exceptionnelles de budget révisé</h4><div class="tablewrap"><table><thead><tr><th>Demandeur</th><th>Donneur</th><th>Montant</th><th>Motif</th><th>Statut</th><th>Décision</th><th></th></tr></thead><tbody>${(m.reallocations||[]).slice().sort((a,b)=>String(b.Date_Demande||'').localeCompare(String(a.Date_Demande||''))).map(rq=>{const gt=m.groups.find(g=>+g.id===+rq.Groupe_Demandeur),gf=m.groups.find(g=>+g.id===+rq.Groupe_Donneur);return `<tr><td><b>${esc(gt?.Nom||'—')}</b></td><td>${esc(gf?.Nom||'—')}</td><td class="num"><b>${money(rq.Montant,'EUR')}</b></td><td>${esc(rq.Motif||'')}</td><td>${rq.Statut==='ACCEPTEE'?'<span class="badge ok">Acceptée</span>':rq.Statut==='REFUSEE'?'<span class="badge danger">Refusée</span>':'<span class="badge warn">En attente</span>'}</td><td>${rq.Date_Decision?esc(String(rq.Date_Decision).slice(0,10)):'—'}</td><td>${ceCanEdit()&&rq.Statut==='EN_ATTENTE'?`<button class="mini-btn" data-ce-accept-realloc="${rq.id}">Accepter</button> <button class="mini-btn ce-danger" data-ce-reject-realloc="${rq.id}">Refuser</button>`:''}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune demande.</td></tr>'}</tbody></table></div><p class="ce-muted">Une réallocation acceptée ne change pas l’affectation initiale. Elle transfère uniquement de la budget révisé d’un groupe vers un autre, ce qui permet de conserver la lecture budget initial / budget révisé / consommation réelle.</p><h4 style="margin-top:20px">Vue par utilisateur</h4><div class="tablewrap"><table><thead><tr><th>Utilisateur</th><th>Organisation</th><th>Groupe</th><th>Plafond mensuel</th><th>Consommé cumulé</th></tr></thead><tbody>${rows.map(x=>`<tr><td><b>${esc(x.r.Nom)}</b><br><span class="ce-muted">${esc(x.r.Email||'')}</span></td><td>${esc(x.o?.Nom||'—')}</td><td>${esc(x.g?.Nom||'—')}</td><td class="num">${money(x.limit,'EUR')}</td><td class="num"><b>${money(x.used,'EUR')}</b></td></tr>`).join('')||'<tr><td colspan="5">Aucune ressource.</td></tr>'}</tbody></table></div></article><article class="card"><h3>Journal des consommations</h3><div class="tablewrap"><table><thead><tr><th>Période</th><th>Ressource</th><th>Groupe</th><th>Organisation</th><th>Montant</th><th>Source</th><th></th></tr></thead><tbody>${m.consumption.slice().sort((a,b)=>String(b.Periode||'').localeCompare(String(a.Periode||''))).map(c=>{const r=m.resources.find(x=>+x.id===+c.Ressource),g=m.groups.find(x=>+x.id===+c.Groupe),o=m.orgs.find(x=>+x.id===+c.Organisation);return `<tr><td>${esc(c.Periode||'—')}</td><td>${esc(r?.Nom||'—')}</td><td>${esc(g?.Nom||'—')}</td><td>${esc(o?.Nom||'—')}</td><td class="num"><b>${money(c.Montant,'EUR')}</b></td><td>${esc(c.Source||'Manuel')}</td><td>${ceCanEdit()?`<button class="mini-btn ce-danger" data-ce-del-cons="${c.id}">Suppr.</button>`:''}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune consommation enregistrée.</td></tr>'}</tbody></table></div></article>`;document.getElementById('ceAddConsumption')?.addEventListener('click',()=>ceConsumptionDialog(m));document.getElementById('ceAddReallocation')?.addEventListener('click',()=>ceReallocationDialog(m));body.querySelectorAll('[data-ce-accept-realloc]').forEach(b=>b.onclick=()=>ceAcceptReallocation(m,(m.reallocations||[]).find(x=>+x.id===+b.dataset.ceAcceptRealloc)));body.querySelectorAll('[data-ce-reject-realloc]').forEach(b=>b.onclick=()=>ceRejectReallocation(m,(m.reallocations||[]).find(x=>+x.id===+b.dataset.ceRejectRealloc)));body.querySelectorAll('[data-ce-del-cons]').forEach(b=>b.onclick=()=>ceDeleteRecord(T.claudeConsumption,+b.dataset.ceDelCons,'consommation'))}
function ceConsumptionDialog(m){if(!m.resources.length){toast('Ajoutez d’abord une ressource.',true);return}const now=new Date(),period=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;ceDialog('Saisir une consommation',`<div class="ce-formgrid"><label class="field full">Ressource<select id="ceFRes">${m.resources.map(r=>`<option value="${r.id}">${esc(r.Nom)}${r.Email?' — '+esc(r.Email):''}</option>`).join('')}</select></label><label class="field">Période (AAAA-MM)<input id="ceFPeriod" value="${period}" placeholder="2026-09"></label><label class="field">Montant réel (€)<input id="ceFAmount" type="number" min="0" step="0.01" value="0"></label><label class="field">Source<input id="ceFSource" value="Manuel" placeholder="API Anthropic, CSV, facture..."></label><label class="field full">Commentaire<input id="ceFComment" value=""></label><div class="field full ce-muted">La saisie est bloquée uniquement si le cumul mensuel de l’utilisateur dépasse sa limite effective (groupe ou dérogation individuelle). Le budget révisé du groupe est un seuil de simulation : son dépassement signale une consommation potentielle de la marge non affectée puis des enveloppes des autres groupes, sans bloquer la saisie.</div></div>`,async d=>{const rid=+d.querySelector('#ceFRes').value,r=m.resources.find(x=>+x.id===rid),g=m.groups.find(x=>+x.id===+r.Groupe),f={Scenario:m.s.id,Organisation:+r.Organisation||0,Groupe:+r.Groupe||0,Ressource:rid,Periode:d.querySelector('#ceFPeriod').value.trim(),Montant:+d.querySelector('#ceFAmount').value||0,Source:d.querySelector('#ceFSource').value.trim()||'Manuel',Commentaire:d.querySelector('#ceFComment').value.trim()};if(!/^\d{4}-\d{2}$/.test(f.Periode))throw Error('La période doit être au format AAAA-MM.');if(f.Montant<0)throw Error('Le montant doit être positif.');const limit=ceLimit(r,m.groups),already=cePeriodUsed(m,rid,f.Periode);if(limit>=0&&already+f.Montant>limit+0.00001)throw Error(`Limite mensuelle dépassée pour ${r.Nom} : ${money(already,'EUR')} déjà consommés + ${money(f.Montant,'EUR')} > ${money(limit,'EUR')}. Activez une dérogation individuelle ou réduisez la saisie.`);await grist.docApi.applyUserActions([['AddRecord',T.claudeConsumption,null,f]])})}
function ceInfoDialog(title,definition,formula,example,note){
  document.querySelector('.ce-info-modal')?.remove();
  const d=document.createElement('div');
  d.className='ce-info-modal';
  d.innerHTML=`<div class="ce-info-panel" role="dialog" aria-modal="true" aria-label="${esc(title)}"><div class="ce-info-panel-head"><h3>${esc(title)}</h3><button type="button" class="ce-info-close" aria-label="Fermer">×</button></div><div class="ce-info-panel-body"><p class="ce-info-definition">${definition}</p>${formula?`<div class="ce-info-formula"><span>Formule</span><b>${formula}</b></div>`:''}${example?`<div class="ce-info-example">${example}</div>`:''}${note?`<div class="ce-info-note"><span>ⓘ</span><p>${note}</p></div>`:''}</div></div>`;
  document.body.appendChild(d);
  const close=()=>d.remove();
  d.querySelector('.ce-info-close').onclick=close;
...
async function ceNewScenario(){const name=prompt('Nom du nouveau scénario Claude Enterprise :','Nouveau scénario');if(!name?.trim())return;const ids=await grist.docApi.applyUserActions([['AddRecord',T.claudeScenarios,null,{Nom:name.trim(),Description:'',Duree_Engagement_Mois:12,Actif:true}]]);await ceReload();const all=ceRows('claudeScenarios');CE_SELECTED_SCENARIO=+all[all.length-1]?.id||CE_SELECTED_SCENARIO;renderClaudeEnterprise()}
async function ceRenameScenario(s){
  ceDialog('Modifier le scénario',`<div class="ce-formgrid">
    <label class="field full">Nom<input id="ceFScenarioName" value="${esc(s.Nom||'')}"></label>
    <label class="field">Durée d’engagement (mois)<input id="ceFScenarioMonths" type="number" min="1" step="1" value="${Math.max(1,+s.Duree_Engagement_Mois||12)}"><span class="ce-muted">Durée contractuelle utilisée pour l’estimation mensuelle.</span></label>
    <label class="field full">Description<textarea id="ceFScenarioDesc">${esc(s.Description||'')}</textarea></label>
  </div>`,async d=>{
    const name=d.querySelector('#ceFScenarioName').value.trim();
    const months=Math.max(1,+d.querySelector('#ceFScenarioMonths').value||12);
    if(!name)throw new Error('Le nom du scénario est obligatoire.');
    await grist.docApi.applyUserActions([['UpdateRecord',T.claudeScenarios,s.id,{Nom:name,Description:d.querySelector('#ceFScenarioDesc').value.trim(),Duree_Engagement_Mois:months}]]);
  });
}
async function ceDuplicateScenario(m){const name=prompt('Nom de la copie :',`${m.s.Nom} - copie`);if(!name?.trim())return;try{await grist.docApi.applyUserActions([['AddRecord',T.claudeScenarios,null,{Nom:name.trim(),Description:m.s.Description||'',Duree_Engagement_Mois:Math.max(1,+m.s.Duree_Engagement_Mois||12),Actif:true}]]);D=await fetchAll();const ns=ceRows('claudeScenarios').slice().sort((a,b)=>+b.id-+a.id)[0];const actions=[],orgMap=new Map(),groupMap=new Map();for(const o of m.orgs){actions.push(['AddRecord',T.claudeOrgs,null,{Scenario:ns.id,Nom:o.Nom,Plafond_Global:+o.Plafond_Global||0,Mode_Facturation:o.Mode_Facturation||'PO',Enveloppe_Engagee:+o.Enveloppe_Engagee||0,Actif:o.Actif!==false,Commentaire:o.Commentaire||''}])}if(actions.length)await grist.docApi.applyUserActions(actions);D=await fetchAll();const newOrgs=ceRows('claudeOrgs').filter(x=>+x.Scenario===+ns.id);m.orgs.forEach(o=>{const n=newOrgs.find(x=>x.Nom===o.Nom&&!Array.from(orgMap.values()).includes(x.id));if(n)orgMap.set(o.id,n.id)});const ga=m.groups.map(g=>['AddRecord',T.claudeGroups,null,{Scenario:ns.id,Organisation:orgMap.get(g.Organisation)||0,Nom:g.Nom,Limite_User_Mois:+g.Limite_User_Mois||0,Duree_Engagement_Mois:ceGroupMonths(m,g),Enveloppe_Allouee:+g.Enveloppe_Allouee||0,Plafond_Groupe_Total:+g.Plafond_Groupe_Total||0,Actif:g.Actif!==false,Commentaire:g.Commentaire||''}]);if(ga.length)await grist.docApi.applyUserActions(ga);D=await fetchAll();const newGroups=ceRows('claudeGroups').filter(x=>+x.Scenario===+ns.id);m.groups.forEach(g=>{const n=newGroups.find(x=>x.Nom===g.Nom&&+x.Organisation===+(orgMap.get(g.Organisation)||0)&&!Array.from(groupMap.values()).includes(x.id));if(n)groupMap.set(g.id,n.id)});const ra=m.resources.map(r=>['AddRecord',T.claudeResources,null,{Scenario:ns.id,Nom:r.Nom,Email:r.Email||'',Organisation:orgMap.get(r.Organisation)||0,Groupe:groupMap.get(r.Groupe)||0,Actif:r.Actif!==false,Limite_Individuelle_Active:!!r.Limite_Individuelle_Active,Limite_Individuelle:+r.Limite_Individuelle||0,Commentaire:r.Commentaire||''}]);if(ra.length)await grist.docApi.applyUserActions(ra);CE_SELECTED_SCENARIO=ns.id;await ceReload();toast('Scénario dupliqué avec sa configuration.')}catch(e){console.error(e);toast('Duplication impossible : '+(e.message||e),true)}}
async function ceDeleteScenario(m){if(!confirm(`Supprimer le scénario « ${m.s.Nom} » et toute sa configuration ?`))return;const actions=[...(m.reallocations||[]).map(r=>['RemoveRecord',T.claudeRealloc,r.id]),...m.consumption.map(c=>['RemoveRecord',T.claudeConsumption,c.id]),...m.resources.map(r=>['RemoveRecord',T.claudeResources,r.id]),...m.groups.map(g=>['RemoveRecord',T.claudeGroups,g.id]),...m.orgs.map(o=>['RemoveRecord',T.claudeOrgs,o.id]),['RemoveRecord',T.claudeScenarios,m.s.id]];await grist.docApi.applyUserActions(actions);CE_SELECTED_SCENARIO=0;await ceReload()}
function ceOrgDialog(m,o=null){ceDialog(o?'Modifier l’organisation':'Nouvelle organisation',`<div class="ce-formgrid"><label class="field full">Nom<input id="ceFName" value="${esc(o?.Nom||'')}"></label><label class="field">Plafond global bloquant (€)<input id="ceFCap" type="number" min="0" value="${+o?.Plafond_Global||0}"></label><label class="field">Mode de facturation<select id="ceFBilling"><option value="PO" ${String(o?.Mode_Facturation||'PO')==='PO'?'selected':''}>Contrat / PO</option><option value="PREPAYE" ${String(o?.Mode_Facturation||'')==='PREPAYE'?'selected':''}>Crédits prépayés</option></select></label><label class="field">Enveloppe engagée (€)<input id="ceFCommitted" type="number" min="0" step="0.01" value="${+o?.Enveloppe_Engagee||0}"></label><label class="field">Actif<select id="ceFActive"><option value="1" ${o?.Actif!==false?'selected':''}>Oui</option><option value="0" ${o?.Actif===false?'selected':''}>Non</option></select></label></div>`,async d=>{const f={Scenario:m.s.id,Nom:d.querySelector('#ceFName').value.trim(),Plafond_Global:+d.querySelector('#ceFCap').value||0,Mode_Facturation:d.querySelector('#ceFBilling').value,Enveloppe_Engagee:+d.querySelector('#ceFCommitted').value||0,Actif:d.querySelector('#ceFActive').value==='1'};if(!f.Nom)throw Error('Le nom est obligatoire.');await grist.docApi.applyUserActions([[o?'UpdateRecord':'AddRecord',T.claudeOrgs,o?.id||null,f]])})}
function ceGroupDialog(m,g=null){if(!m.orgs.length){toast('Créez d’abord une organisation.',true);return}ceDialog(g?'Modifier le groupe':'Nouveau groupe',`<div class="ce-formgrid"><label class="field">Organisation<select id="ceFOrg">${m.orgs.map(o=>`<option value="${o.id}" ${+g?.Organisation===+o.id?'selected':''}>${esc(o.Nom)}</option>`).join('')}</select></label><label class="field">Nom<input id="ceFName" value="${esc(g?.Nom||'')}"></label><label class="field">Limite / utilisateur / mois (€)<input id="ceFLimit" type="number" min="0" value="${+g?.Limite_User_Mois||0}"></label><label class="field">Durée d’engagement (mois)<input id="ceFGroupMonths" type="number" min="1" step="1" value="${ceGroupMonths(m,g)}"><span class="ce-muted">Utilisée pour calculer le budget mensuel estimé du groupe.</span></label><label class="field">Affectation initiale (€)<input id="ceFAllocated" type="number" min="0" step="0.01" value="${+g?.Enveloppe_Allouee||0}"></label><label class="field">Budget révisé simulé du groupe (€)<input id="ceFGroupCap" type="number" min="0" step="0.01" value="${+g?.Plafond_Groupe_Total||0}"><span class="ce-muted">Doit être ≥ à l’affectation. L’écart budget révisé − affectation initiale représente la consommation potentielle de la réserve commune / des autres groupes.</span></label><label class="field">Actif<select id="ceFActive"><option value="1" ${g?.Actif!==false?'selected':''}>Oui</option><option value="0" ${g?.Actif===false?'selected':''}>Non</option></select></label></div>`,async d=>{const orgId=+d.querySelector('#ceFOrg').value,allocated=+d.querySelector('#ceFAllocated').value||0,groupCap=+d.querySelector('#ceFGroupCap').value||0,f={Scenario:m.s.id,Organisation:orgId,Nom:d.querySelector('#ceFName').value.trim(),Limite_User_Mois:+d.querySelector('#ceFLimit').value||0,Duree_Engagement_Mois:Math.max(1,+d.querySelector('#ceFGroupMonths').value||12),Enveloppe_Allouee:allocated,Plafond_Groupe_Total:groupCap,Actif:d.querySelector('#ceFActive').value==='1'};if(!f.Nom)throw Error('Le nom est obligatoire.');if(groupCap&&groupCap<allocated)throw Error('Le budget révisé total ne peut pas être inférieur à l’affectation initiale.');const org=m.orgs.find(o=>+o.id===orgId),other=m.groups.filter(x=>+x.Organisation===orgId&&+x.id!==+g?.id).reduce((a,x)=>a+Math.max(0,+x.Enveloppe_Allouee||0),0),engaged=Math.max(0,+org?.Enveloppe_Engagee||0);if(engaged&&other+allocated>engaged+0.00001)throw Error(`La somme affectée aux groupes (${money(other+allocated,'EUR')}) dépasse l’enveloppe engagée de l’organisation (${money(engaged,'EUR')}).`);await grist.docApi.applyUserActions([[g?'UpdateRecord':'AddRecord',T.claudeGroups,g?.id||null,f]])})}
function ceResourceDialog(m,r=null){if(!m.groups.length){toast('Créez d’abord une organisation et un groupe.',true);return}ceDialog(r?'Modifier la ressource':'Nouvelle ressource',`<div class="ce-formgrid"><label class="field">Nom<input id="ceFName" value="${esc(r?.Nom||'')}"></label><label class="field">Email<input id="ceFEmail" value="${esc(r?.Email||'')}"></label><label class="field">Groupe<select id="ceFGroup">${m.groups.map(g=>{const o=m.orgs.find(x=>+x.id===+g.Organisation);return `<option value="${g.id}" ${+r?.Groupe===+g.id?'selected':''}>${esc(o?.Nom||'')} — ${esc(g.Nom)}</option>`}).join('')}</select></label><label class="field">Statut<select id="ceFActive"><option value="1" ${r?.Actif!==false?'selected':''}>Actif</option><option value="0" ${r?.Actif===false?'selected':''}>Inactif</option></select></label><label class="field">Dérogation individuelle<select id="ceFOverride"><option value="0" ${!r?.Limite_Individuelle_Active?'selected':''}>Non — hériter du groupe</option><option value="1" ${r?.Limite_Individuelle_Active?'selected':''}>Oui</option></select></label><label class="field">Limite spécifique (€ / mois)<input id="ceFIndividual" type="number" min="0" value="${+r?.Limite_Individuelle||0}"></label><div class="field full ce-muted" id="ceFEffective"></div></div>`,async d=>{const gid=+d.querySelector('#ceFGroup').value,g=m.groups.find(x=>+x.id===gid),override=d.querySelector('#ceFOverride').value==='1',f={Scenario:m.s.id,Nom:d.querySelector('#ceFName').value.trim(),Email:d.querySelector('#ceFEmail').value.trim(),Organisation:+g.Organisation,Groupe:gid,Actif:d.querySelector('#ceFActive').value==='1',Limite_Individuelle_Active:override,Limite_Individuelle:+d.querySelector('#ceFIndividual').value||0};if(!f.Nom)throw Error('Le nom est obligatoire.');await grist.docApi.applyUserActions([[r?'UpdateRecord':'AddRecord',T.claudeResources,r?.id||null,f]])});setTimeout(()=>{const d=document.querySelector('.ce-dialog:last-of-type');if(!d)return;const group=d.querySelector('#ceFGroup'),ov=d.querySelector('#ceFOverride'),ind=d.querySelector('#ceFIndividual'),info=d.querySelector('#ceFEffective');const refresh=()=>{const g=m.groups.find(x=>+x.id===+group.value),override=ov.value==='1',limit=override?(+ind.value||0):(+g?.Limite_User_Mois||0);ind.disabled=!override;info.innerHTML=`Limite effective : <b>${money(limit,'EUR')} / mois</b> — ${override?'dérogation individuelle':'héritée automatiquement du groupe '+esc(g?.Nom||'')}.`;};group.onchange=refresh;ov.onchange=refresh;ind.oninput=refresh;refresh()},0)}
