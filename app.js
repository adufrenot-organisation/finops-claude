...
function ceRenderOverview(m,pctCap){
  ...
  const info=(label,text)=>`${label} <button type="button" class="ce-info-btn" data-ce-info="${encodeURIComponent(JSON.stringify({label,text}))}" aria-label="Afficher l’explication de ${esc(label)}">ⓘ</button>`;
  ...
  body.innerHTML=`<div class="ce-layout"><article class="card"><div class="ce-hero"><div><h3>Pilotage budgétaire consolidé</h3><p>Affectation initiale, budget révisé, consommation réelle et trajectoire prévisionnelle.</p></div></div>
  <div class="kpis">
    <div class="kpi"><div class="v">${money(monthlyEstimate,'EUR')}</div><div class="l">${info('Budget mensuel estimé','Somme, pour chaque groupe, de : affectation initiale ÷ durée d’engagement du groupe.')}</div></div>
    <div class="kpi"><div class="v">${money(usedTotal,'EUR')}</div><div class="l">Consommation réelle</div></div>
    <div class="kpi"><div class="v ${globalBurn>1.15?'negative':''}">${pct(globalBurn)}</div><div class="l">${info('Burn rate global',`Compare le rythme réel au rythme du budget révisé. 100 % = rythme conforme ; 120 % = consommation 20 % trop rapide. Calcul actuel : moyenne consommée ${money(globalAvg,'EUR')}/mois ÷ cible ${money(globalMonthlyTarget,'EUR')}/mois.`)}</div></div>
    <div class="kpi"><div class="v ${landingTotal>currentCaps?'negative':''}">${money(landingTotal,'EUR')}</div><div class="l">${info('Atterrissage',`Estimation de la consommation totale en fin d’engagement si le rythme moyen observé se poursuit. Calcul : consommation moyenne mensuelle observée × durée totale.`)}</div></div>
    <div class="kpi"><div class="v">${globalExhaust}</div><div class="l">${info('Date estimée d’épuisement',`Date à laquelle le plafond global serait épuisé si le rythme moyen de consommation observé se maintient.`)}</div></div>
    <div class="kpi"><div class="v ${riskRows?'negative':''}">${riskRows}</div><div class="l">Groupes à surveiller</div></div>
  </div>
  <div class="ce-bar"><i style="width:${Math.min(100,m.cap?usedTotal/m.cap*100:0)}%"></i></div><div class="ce-muted" style="margin:8px 0 16px">${m.cap?pct(usedTotal/m.cap):'—'} du plafond global consommé · reste ${money(remainingGlobal,'EUR')}.</div>
  <div class="tablewrap"><table><thead><tr><th>Groupe</th><th>Affectation initiale</th><th>Budget révisé</th><th>${info('Budget mensuel','Affectation initiale ÷ durée d’engagement du groupe.')}</th><th>Consommé</th><th>${info('Burn rate','Consommation réelle cumulée ÷ budget révisé théorique cumulé à date. 100 % = rythme conforme.')}</th><th>${info('Atterrissage','Moyenne mensuelle réellement consommée × durée d’engagement.')}</th><th>${info('Besoin / marge','Besoin = atterrissage − budget révisé. Marge = budget révisé − atterrissage.')}</th><th>${info('Épuisement','Date estimée d’épuisement du budget révisé au rythme actuel.')}</th><th>Santé</th></tr></thead><tbody>
  ${rows.map(x=>`<tr><td><b>${esc(x.g.Nom)}</b><br><span class="ce-muted">${esc(x.o?.Nom||'—')} · ${x.elapsed}/${x.months} mois</span></td><td class="num">${money(x.allocated,'EUR')}</td><td class="num"><b>${money(x.currentCap,'EUR')}</b><br><span class="ce-muted">${x.reallocNet>=0?'+':''}${money(x.reallocNet,'EUR')} réalloué</span></td><td class="num">${money(x.monthlyBudget,'EUR')}</td><td class="num">${money(x.used,'EUR')}</td><td class="num ${x.burnRate>1.15?'ce-danger':''}"><b>${pct(x.burnRate)}</b><br><span class="ce-muted">${money(x.avgMonthly,'EUR')}/mois</span></td><td class="num ${x.landing>x.currentCap?'ce-danger':''}"><b>${money(x.landing,'EUR')}</b></td><td class="num">${x.need>0?`<span class="ce-danger">+${money(x.need,'EUR')} besoin</span>`:`<span>−${money(x.releasable,'EUR')} marge</span>`}</td><td>${x.exhaustDate}</td><td>${x.state==='danger'?'<span class="badge danger">Critique</span>':x.state==='warn'?'<span class="badge warn">À surveiller</span>':'<span class="badge ok">Sain</span>'}</td></tr>`).join('')||'<tr><td colspan="10">Aucun groupe.</td></tr>'}
  </tbody></table></div>${recHtml}
  <h4 style="margin-top:22px">${info('Écart cumulé à fin de mois','Consommation réelle cumulée − budget initial théorique cumulé. Positif = consommation en avance sur la trajectoire prévue ; négatif = sous-consommation.')}</h4>
  <div class="tablewrap"><table><thead><tr><th>Groupe</th><th>Affectation initiale</th>${monthlyHeaders}</tr></thead><tbody>${monthlyRows||`<tr><td colspan="${2+periods.length}">Aucun groupe.</td></tr>`}</tbody></table></div>
  </article><article class="card"><h3>Architecture budgétaire</h3><p>Répartition par organisation et groupe.</p>${m.orgs.map(o=>{const gs=m.groups.filter(g=>+g.Organisation===+o.id),used=gs.reduce((a,g)=>a+ceGroupUsed(m,g.id),0);return `<div class="ce-org"><b>${esc(o.Nom)}</b><span class="ce-muted"> · plafond global bloquant ${money(o.Plafond_Global,'EUR')}</span><div class="ce-muted">Consommé ${money(used,'EUR')} · reste avant blocage ${money((+o.Plafond_Global||0)-used,'EUR')}</div><div class="ce-groups">${gs.map(g=>{const rs=m.active.filter(r=>+r.Groupe===+g.id),ex=rs.reduce((a,r)=>a+ceLimit(r,m.groups),0);return `<div class="ce-group"><b>${esc(g.Nom)}</b><div class="ce-muted">${rs.length} ressource(s) · ${ceGroupMonths(m,g)} mois</div><div>${money(g.Limite_User_Mois,'EUR')} / user / mois</div><div class="ce-muted">Exposition mensuelle utilisateurs : <b>${money(ex,'EUR')}</b></div></div>`}).join('')||'<span class="ce-muted">Aucun groupe</span>'}</div></div>`}).join('')||'<div class="ce-muted">Aucune organisation.</div>'}</article></div>`;
