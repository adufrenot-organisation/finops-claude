...
function ceRenderOverview(m,pctCap){
  ...
  const recHtml=totalNeed?`<div class="ce-reco"><b>Recommandations FinOps</b><div class="ce-reco-grid"><div>${needs.map(x=>`<div>🔴 <b>${esc(x.g.Nom)}</b> : besoin prévisionnel <b>${money(x.need,'EUR')}</b></div>`).join('')}</div><div>${donors.map(x=>`<div>🟢 <b>${esc(x.g.Nom)}</b> : capacité potentiellement libérable <b>${money(x.releasable,'EUR')}</b></div>`).join('')||'<div class="ce-muted">Aucune capacité libérable détectée.</div>'}</div></div><div style="margin-top:10px">Capacité réallouable détectée : <b>${money(Math.min(totalNeed,totalRelease),'EUR')}</b>${residual?` · besoin résiduel : <b class="ce-danger">${money(residual,'EUR')}</b>`:' · <b>besoin théoriquement absorbable</b>'}</div></div>`:`<div class="ce-reco"><b>Recommandations FinOps</b><div class="ce-muted">Aucun dépassement prévisionnel détecté au rythme actuel.</div></div>`;
