  ...
  return `<div class="scenario-team-budget">
    ...
    <div class="scenario-team-totals">
      ${x.rows.map(r=>{
        ...
        return `<div class="scenario-team-total-card">
          <span>${esc(r.team)}</span>
          <small>${num(r.licenses)} licence(s) toutes offres</small>
          <div class="scenario-team-card-split">
            <span>Fixe <b>${money(r.fixed)}</b><small>≈ ${money(r.fixed*(+m.rate||0),'EUR')}</small></span>
            <span>Variable <b>${money(r.variable)}</b><small>≈ ${money(r.variable*(+m.rate||0),'EUR')}</small></span>
          </div>
          <b class="scenario-team-card-total">Total ${money(r.budget)} <small>≈ ${money(r.budget*(+m.rate||0),'EUR')}</small></b>
          <div class="scenario-team-roi">
            ...
            <div class="scenario-team-roi-grid">
              ...
              <span><small>Économie RH</small><b>${money(hrSaving,'EUR')}</b></span>
              <span><small>Coût annuel licences</small><b>${money(lic,'EUR')}</b></span>
              <span class="${gain<0?'negative':''}"><small>Gain net annuel</small><b>${money(gain,'EUR')}</b></span>
              <span class="scenario-team-roi-primary ${roiPct<0?'negative':''}"><small>ROI / gain %</small><b>${pct(roiPct)}</b></span>
            </div>
          </div>
        </div>`;
      }).join("")}
